...
- [ ] (2026-09-24) Hash pinning (A19), on branch `feature/pin-by-hash`: `docs/adr/0003`, README and setup guide show hash-pinned snippets, `src/pinning.test.ts` (both tests seen red before the fix), `ci.yml` and `deploy.yml` pinned (`actions/checkout` v4.4.0, `oven-sh/setup-bun` v2.2.0 — the commits `v4` and `v2` pointed at, so no behaviour change), `.github/dependabot.yml`; immutable releases enabled by the user. Remaining: the `v1.0.0` Release on `b706447` (awaiting the user's approval), then `gakumas-supportcards` switches from `2f92e97` to that hash.
